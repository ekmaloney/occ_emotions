...
m3_5 <- glmer(count ~ dist_emotion + mat_power + sex + race + age_std + (1 + dist_emotion| id)+ (1|emotion), data = gss_long_nomiss, family = poisson())

m4 <- glmer(count ~ dist_emotion*mat_power + sex + age_std + race + (1 + dist_emotion| id)+ (1|emotion), data = gss_long_nomiss, family = poisson(), control = lmerControl(optimizer = "bobyqa"))

m5 <- glmer(count ~ dist_emotion*sex + dist_emotion*mat_power + race + age_std + (1 + dist_emotion | id)+ (1|emotion), data = gss_long_nomiss, family = poisson())

m6 <- glmer(count ~ dist_emotion*race + dist_emotion*mat_power + dist_emotion*sex + age_std + (1 + dist_emotion | id)+ (1|emotion), data = gss_long_nomiss, family = poisson(), control = lmerControl(optimizer = "bobyqa"))

m7 <- glmer(count ~ dist_emotion*age_std + dist_emotion*sex + race + dist_emotion*mat_power + (1 + dist_emotion + dist_emotion| id)+ (1|emotion), data = gss_long_nomiss, family = poisson(), control = lmerControl(optimizer = "bobyqa"))
...
ncores <- detectCores()
diff_optims <- allFit(m6, maxfun = 1e5, parallel = 'multicore', ncpus = ncores)
...
ncores <- detectCores()
diff_optims <- allFit(m7, maxfun = 1e5, parallel = 'multicore', ncpus = ncores)
