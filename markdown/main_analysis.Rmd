...
skew_scores <- tibble(log_income = skewness(gss_block_emotions$log_income, na.rm = TRUE),
                      dadsei = skewness(gss_block_emotions$dadsei, na.rm = TRUE),
                      education = skewness(gss_block_emotions$educ, na.rm = TRUE),
                      prestige = skewness(gss_block_emotions$prestg80, na.rm = TRUE))

saveRDS(skew_scores, "/Users/emilymaloney/Dropbox/Grad_School/Non-class Research/occ_emotions/output/skew_scores.RDS")

income_hist <- ggplot(gss_block_emotions, mapping = aes(x = log_income)) + 
               geom_histogram(binwidth = 0.2, fill = muted("pink")) +
               theme_minimal() +
               labs(x = "Income (logged)")

sei_hist <- ggplot(gss_block_emotions, mapping = aes(x = dadsei)) + 
               geom_histogram(binwidth = 0.2, fill = muted("pink")) +
               theme_minimal() +
               labs(x = "Father SEI")

educ_hist <- ggplot(gss_block_emotions, mapping = aes(x = educ)) + 
               geom_histogram(binwidth = 1, fill = muted("pink")) +
               theme_minimal() +
               labs(x = "Education (years)")

prest_hist <- ggplot(gss_block_emotions, mapping = aes(x = prestg80)) + 
               geom_histogram(binwidth = 1, fill = muted("pink")) +
               theme_minimal() +
               labs(x = "Occupational Prestige Score")
               
ggsave("/Users/emilymaloney/Dropbox/Grad_School/Non-class Research/occ_emotions/output/income_hist.png", income_hist)
ggsave("/Users/emilymaloney/Dropbox/Grad_School/Non-class Research/occ_emotions/output/sei_hist.png", sei_hist)
ggsave("/Users/emilymaloney/Dropbox/Grad_School/Non-class Research/occ_emotions/output/educ_hist.png", educ_hist)
ggsave("/Users/emilymaloney/Dropbox/Grad_School/Non-class Research/occ_emotions/output/prest_hist.png", prest_hist)
...
rownames(cfa_des_long) <- c( "Father SEI", "Education (years)","Income (log)", "Occupational Prestige Score")
...
tbl_summary(descriptives, 
  ...
  as_kable(caption = "Descriptive Statistics", format = "pandoc")
```
...
Seven total models were estimated to test the hypotheses.[^3] First, the reported count of emotions was regressed on the standardized measure of distance in EPA space from the characteristic emotion, to test H1 without controls. Next, model two includes material power as an additional explanatory variable. Model three includes sex, race, and age as additional covariates, to test whether H1 holds when controlling for these possible confounders. To test H2, the correlation between the random intercepts for the 16 emotions of model 1 and the average distance between occupational identity’s characteristic emotion and the emotion is computed.\ 
...
stargazer(m1, m2, m3, m4,
          ...
          covariate.labels = c("Distance from Emotion",
                               ...
                               "Constant"),
          header=FALSE,
          column.labels=c("M1", "M2", "M3", "M4"), model.numbers=FALSE)

m1_res <- tidy(m1)

dist_beta <- m1_res$estimate[2]

m3_res <- tidy(m3)

pred_data <- tibble(dist_emotion = c(rep(0, 12976), rep(1, 12976)),
                    mat_power = mean(gss_long_nomiss$mat_power),
                    sex = "Male",
                    race = "White",
                    age_std = 0,
                    id = rep(rep(unique(gss_long_nomiss$id),each = 16), 2),
                    emotion = rep(emotion_vars, 1622)) %>% 
              mutate(predicted_counts = predict(m3, newdata = ., allow.new.levels=T),
                     predicted_counts = exp(predicted_counts)) %>%
              distinct() %>% 
              pivot_wider(names_from = dist_emotion,
                          values_from = predicted_counts,
                          names_prefix = "dist_emotion_") %>% 
              mutate(diff = dist_emotion_0 - dist_emotion_1) %>% 
              group_by(emotion) %>% 
              summarise(mdiff = mean(diff)) %>% 
              mutate(m_emo = mean(mdiff))

ints <- ranef(m1)$emotion

ints <- cbind(emo_desc, ints)

int_cor <- cor(ints$mean_dist, ints$`(Intercept)`)
```

Table 6 displays the results from the first four models. Model one shows a statistically significant effect of the distance from characteristic emotion on the expected count of a participant experiencing the GSS emotion. Specifically, with each one standard deviation increase in the distance between an individual’s occupational identity’s characteristic emotion from a GSS emotion word in EPA space, the expected count is `r exp(dist_beta) %>% round(digits = 2)` times as great.\ 

The results from model two indicate that this effect remans negative and significant even after material power is added to the model, suggesting that regardless of one’s resources,  EPA distance from an occupation’s characteristic emotion remains an important deterrent from experiencing that emotion. Interestingly, material power on its own has no direct effect on the emotion counts.  Further, when all other controls are added in model three, distance from emotion remains both negative and significant, only slightly lower in magnitude than in model one. To provide an illustrative example: when everything else is held constant at its mean, we would expect that an individual whose occupational identity’s characteristic emotion is located at the mean distance from the emotion in EPA space (zero on the standardized scale) to report experiencing an emotion `r pred_data %>% filter(emotion == "hapfeel") %>% select(m_emo) %>% round(digits = 3)` fewer days of the week, on average across individuals and emotions. Looking at emotions separately, I predict that for an individual whose characteristic emotion is 1 standard deviation away from "happy" in EPA space, they will report feeling the emotion `r pred_data %>% filter(emotion == "hapfeel") %>% select(mdiff) %>% round(digits = 3)` fewer days than someone whose distance from happy in EPA space is at the mean. Although the magnitude of the effect is small, it is significant. In the following models I explore the hypotheses suggesting that the small size of the effect could be due to the variation in this effect across material resources and status characteristics.\ 
...
stargazer(m5, m6, m7,
          ...
          covariate.labels = c("Distance from Emotion",
                               ...
                               "Constant"),
          header=FALSE,
          column.labels=c("M5", "M6", "M7"), model.numbers=FALSE)
```
...
Although the long format of the data and using a metric of distance from an individual occupational identity's characteristic emotion nicely tests the central hypotheses, it obscures how exactly the predictions look different for individuals whose occupational identities are in diverse EPA spaces. To provide a few illustrative examples, table 8 shows predicted levels of emotions for an individual who is a Firefighter(3.27, 2.85, 2.29); Bill Collector (-1.07, 1.63, 1.02); and a Professor(1.83, 2.27, 0.24). Because these occupational identities exist in different places in EPA space, they are fairly different distances from the emotion words in question, which is shown in the different predictions across emotions. I use model 7 to predict these values, and hold everything else at the mean. Bill Collectors are predicted to experience the negative emotions the most (angry, outraged) while Firefighters are predicted to experience positive emotions the most (excited, happy).\ 

```{r}
gss_emot <- unique(gss_long_nomiss$emotion)

fire_distances <- gss_long_nomiss %>%
                  select(emotion, occ, mdist, dist_emotion) %>% 
                  filter(occ == "firefighter") %>% 
                  distinct()

bill_dist <- gss_long_nomiss %>%
                  select(emotion, occ, mdist, dist_emotion) %>% 
                  filter(occ == "bill_collector") %>% 
                  distinct()

prof_dist <- gss_long_nomiss %>%
                  select(emotion, occ, mdist, dist_emotion) %>% 
                  filter(occ == "professor") %>% 
                  distinct()
  
fpreds <- tibble(dist_emotion = rep(fire_distances$dist_emotion, 811),
                    mat_power = mean(gss_long_nomiss$mat_power),
                    sex = "Male",
                    race = "White",
                    age_std = 0,
                    id = rep(unique(gss_long_nomiss$id), each = 16),
                    emotion = rep(fire_distances$emotion, 811)) %>% 
              mutate(predicted_counts = predict(m7, newdata = ., allow.new.levels=T),
                     predicted_counts_exp = exp(predicted_counts)) %>% 
              group_by(emotion) %>% 
              summarise(firefighter = mean(predicted_counts_exp))

bpreds <- tibble(dist_emotion = rep(bill_dist$dist_emotion, 811),
                    mat_power = mean(gss_long_nomiss$mat_power),
                    sex = "Male",
                    race = "White",
                    age_std = 0,
                    id = rep(unique(gss_long_nomiss$id), each = 16),
                    emotion = rep(bill_dist$emotion, 811)) %>% 
              mutate(predicted_counts = predict(m7, newdata = ., allow.new.levels=T),
                     predicted_counts_exp = exp(predicted_counts)) %>% 
              group_by(emotion) %>% 
              summarise(billcollector = mean(predicted_counts_exp))

ppreds <- tibble(dist_emotion = rep(prof_dist$dist_emotion, 811),
                    mat_power = mean(gss_long_nomiss$mat_power),
                    sex = "Male",
                    race = "White",
                    age_std = 0,
                    id = rep(unique(gss_long_nomiss$id), each = 16),
                    emotion = rep(prof_dist$emotion, 811)) %>% 
              mutate(predicted_counts = predict(m7, newdata = ., allow.new.levels=T),
                     predicted_counts_exp = exp(predicted_counts)) %>% 
              group_by(emotion) %>% 
              summarise(professor = mean(predicted_counts_exp))

all_predictions <- left_join(fpreds, bpreds)
all_predictions <- left_join(all_predictions, ppreds)
all_predictions <- all_predictions %>% 
                   mutate(emotion = str_replace(emotion, "happy", "hapfeel"),
                          emotion = str_replace(emotion, "overjoyed", "ovrjoyed"),
                          emotion = str_replace(emotion, "contented", "contentd"),
                          emotion = str_replace(emotion, "mad", "madat"))

kable(all_predictions, digits = 3, caption = "Predicted Emotions for 3 Occupations", format = "pandoc")
```
...
In terms of prior work on the intersection of emotion and experience, this paper provides a different lens through which to view the link: identity, which has been shown to helpfully bridge structural and cultural explanations of emotional experience in the past (Collett and Lizardo 2010; Lively and Powell 2016). While the results found here cannot explain all of the questions left open in previous work on emotions on the job, it affords a general framework to use when thinking about how occupations shape one’s emotional experience. For instance, when thinking about emotional labor and emotion management, concepts such as feeling rules and emotion norms could be formalized by using Affect Control Theory’s measurement tools which can quantify cultural expectations (Freeland and Hoey 2018; Hunzaker 2016).\ 

Additionally, the results indicating the influence of material and social interactional resources supports prior work on how resources aid in interaction management and defining the situation, and provide impetus for further work on how exactly these mechanisms work as situations unfold. In particular, this work suggests that fruitful avenues of research investigate whether definition of the situation is taken by the higher status person or given through deference actions of the lower status individual. Additionally, future work could look at which resources are more or less effective at maintaining an interaction and how different resources work together to maintain a situation.\ 
...
[^3]: A full replication package, including data and code for all of the analysis can be found at: [blinded] github repository. 
