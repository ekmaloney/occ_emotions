...
gss_long_nomiss <- gss_long_emotions %>% 
                   filter(!is.na(count) & !is.na(sex) & !is.na(age) & !is.na(race) & !is.na(childs) & 
...
gss_long_nomiss <- gss_long_nomiss %>% 
                   mutate(age_cat = case_when(age >= 18 & age <= 29 ~ "18-29",
                                              ...
                                              age > 44 & age <= 64 ~ "45-64",
                                              age < 65 ~ "65+"),
                          pos_emotion = if_else(emotion %in% c("happy", "overjoyed", "calm",
                                                                 "excited", "interested", "contented", "proud"), 
                                                  1, 0))
library(lme4)
library(stargazer)


new_test <- glmer(count ~ dist_emotion + income_std*emotion + sex*emotion + race*emotion + 
                    age_cat*emotion + (1 | id), 
            data = gss_long_nomiss, family = poisson(),
            control = glmerControl(optimizer = "bobyqa"))

new_test_2 <- glmer(count ~ dist_emotion*pos_emotion + income_std*emotion + sex*emotion + race*emotion + 
                      age_cat*emotion + (1 | id), 
            data = gss_long_nomiss, family = poisson(),
            control = glmerControl(optimizer = "bobyqa"))
